...
export const getMessages = async (userId1, userId2, limit = 50, cursor = null) => {
    const where = {
        OR: [
...
export const getConversation = async (userId) => {
    ...
    // Fetch all messages between you and your friend 
    ...
    // Figure out last message and unread count per friend
    ...
    allMessages.forEach(msg => {
        ...
        if(!lastMessage.has(partnerId)){
            lastMessage.set(partnerId, msg);
        }
    ...
    const conversations = friends.map(friend => ({
        ...friend,
        lastMessage: lastMessage.get(friend.id) || null,
